...
module.exports = {
  checkFtpFolder: () => {
    ...
    ftp.on("ready", async () => {
      ...
      let activedStation = findActiveStation(
        arrivedFolder,
        databaseInfo.stationInfo
      )
      ...
      await Promise.all(
        activedStation.map(async station => {
          ...
          console.log(
            chalk.yellow(
              `------- 3. Find files in Station Folder ${station.name}------------`
            )
          )
          // console.log({files})
          let test = await Promise.all(
            files.map(async file => {
              ...
              await readFileFtp(
                ...
                file,
                station,
                databaseInfo.systemInfo,
                databaseInfo.thresholdInfo
              )
              // await testreadFileFtp(ftp, file, station, databaseInfo.systemInfo, databaseInfo.thresholdInfo)
            })
          )
          console.log(
            chalk.red(
              `------- Out of read File FTP ${station.name}------------`
            )
          )
        })
      )
...
function listFilesFtp(ftp, station, systemInfo) {
  return new Promise((resolve, reject) => {
    let dirStation = `${func.getValueSystem(systemInfo, "dirReceiveFtp")}/${
      station.ftpFolder
    }`
    ftp.list(dirStation, (err, result) => {
      ...
      let listFiles = []
      console.log(
        chalk.yellow(`------- 2. Check Disconnection  ${station.name} --------`)
      )
      if (result === undefined || !result.length) {
        ...
        if (timeSubtract > 60) {
          // notify ko truyen du lieu
          app.Notification.addNotification(station, "STATION_NOT_SEND_DATA")
          ...
          if (sendAlertDisconnection(station, systemInfo)) {
            ...
            models.Station.updateStationById(
              stationAlertFields,
              station.id
            ).then(result => {
              console.log(
...
async function readFileFtp(
  ...
  // console.log({station})
  let dirFile = `${func.getValueSystem(systemInfo, "dirReceiveFtp")}/${
    station.ftpFolder
  }/${fileName}`
  // console.log({dirFile})
  ...
  // console.log({data})
  console.log(
    chalk.yellow(`-------- 4.1 Done Stream File Ftp ${station.name}--------`)
  )
  await delete_FtpFile(ftp, dirFile)
  console.log(
  ...
  if (!validateFileName) {
    ...
    if (sendAlertWrongStructure(station, systemInfo)) {
      // notify ko dung cau truc du lieu
      let alertCount = station.numberOfAlertStructure + 1
      let stationAlertFields = { numberOfAlertStructure: alertCount }
      let dirFolder = await checkFtpFileFolder(
        ftp,
        fileName,
        station,
        func.getValueSystem(systemInfo, "dirWrongFtp")
      )
      console.log({ dirFolder })
      await putFileFTP(ftp, data, `${dirFolder}/${fileName}`)
      models.Station.updateStationById(stationAlertFields, station.id).then(
        result => {
          console.log(
            `Đã update số lần cảnh báo của tram ${station.name} lên ${alertCount}`
          )
  ...
  let insertedInfo = await models.MonitoringDataInfo.createMonitoringDataInfo(
    dataInfo
  )
  ...
  if (insertedInfo.id !== undefined) {
    ...
    console.log(
      chalk.yellow(`-------- 6. Check Threshold ${station.name} --------`)
    )
    ...
    if (filterResult.length > 0) {
      ...
      models.Station.updateStationById(stationFiles, station.id).then(
        result => {
          console.log(
            `Đã update số lần vượt ngưỡng của tram ${station.name} lên ${countOverThreshold}`
          )
          console.log({ result })
        }
      )
    ...
    let dirFolder = await checkFtpFileFolder(
      ...
      fileName,
      station,
      func.getValueSystem(systemInfo, "dirSaveFtp")
    )
    console.log({ dirFolder })
    await putFileFTP(ftp, data, `${dirFolder}/${fileName}`)
  }
...
async function testreadFileFtp(
  ftp,
  fileName,
  station,
  systemInfo,
  ...
  await checkFtpFileFolder(ftp, fileName, station, systemInfo)
}
...
async function checkFtpFileFolder(ftp, fileName, station, rootFolder) {
  try {
    ...
    let typeFolder = ""
    // await createFtpFolder(ftp, func.getValueSystem(systemInfo, 'dirWrongFtp'), station.ftpFolder)
    await createFtpFolder(ftp, rootFolder, station.ftpFolder)
    if (station.monitoringType == "QTN") {
      typeFolder = "nuoc"
    } else {
      typeFolder = "khi"
    }
    // let folderStation = `${func.getValueSystem(systemInfo, 'dirWrongFtp')}/${station.ftpFolder}`
    let folderStation = `${rootFolder}/${station.ftpFolder}`
    await createFtpFolder(ftp, folderStation, typeFolder)
    let folderRootType = `${folderStation}/${typeFolder}`
    await createFtpFolder(ftp, folderRootType, `${fileInfo.year}`)
    await createFtpFolder(
      ftp,
      `${folderRootType}/${fileInfo.year}`,
      `Thang${fileInfo.month}`
    )
    ...
    await createFtpFolder(ftp, dirMonthFolder, dirTimeFolder)
    console.log(`${dirMonthFolder}/${dirTimeFolder}`)
    return `${dirMonthFolder}/${dirTimeFolder}`
    // await putFileFTP(ftp, 'abc', `${dirMonthFolder}/${dirTimeFolder}/${fileName}`)
  } catch (error) {
...
async function createFtpFolder(ftp, rootDir, folderDir) {
  ...
  if (!listFolder1.includes(folderDir)) {
    await mkdirFtp(ftp, `${rootDir}/${folderDir}`)
  }
}

function mkdirFtp(ftp, dirFolder) {
  return new Promise((resolve, reject) => {
    ftp.mkdir(dirFolder, err => {
      if (err) reject(err)
...
function delete_FtpFile(ftp, path) {
  return new Promise((resolve, reject) => {
    ftp.delete(path, err => {
      if (err) reject(err)
      resolve()
...
function getTimeInfo(time) {
  let timeInfo = {}
  timeInfo.year = `${time.substr(0, 4)}`
  timeInfo.month = `Thang${time.substr(4, 2)}`
  timeInfo.day = `${time.substr(6, 2)}${time.substr(4, 2)}${time.substr(0, 4)}`
  return timeInfo
}
